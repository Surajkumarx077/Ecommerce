...
// Update session
    ...
    public String checkout(@ModelAttribute("cart") List<Product> cart,
        ...
        order.setUserEmail(authentication.name());
...
// Create order item
            ...
            item.setOrder((jakarta.persistence.criteria.Order) order);
            total += dbProduct.getPrice();
            order.getClass().cast(item);
        }
...
// Send email
    ...
    public String userOrders(Model model, Authentication authentication) {
        List<jakarta.persistence.criteria.Order> orders = orderRepository.findByUserEmail(authentication.name());
